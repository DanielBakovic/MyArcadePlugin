      ...
      <table class="optiontable" width="100%" cellpadding="5" cellspacing="5">
        ...
        <tr>
          <td>
            <select size="1" name="gamemonetize_category" id="gamemonetize_category">
							...
							<option value="19" <?php myarcade_selected( $gamemonetize['category'], '19' ); ?> ><?php _e( 'Baby Hazel', 'myarcadeplugin' ); ?></option>
        ...
        <tr>
          <td>
					...
					<td><i><?php _e( 'Enable this if you want to fetch games automatically. Navigate to General Settings to select a cron interval.', 'myarcadeplugin' ); ?></i></td>
        </tr>
        ...
        <tr>
          <td>
					...
					<td><i><?php _e( 'Enable this if you want to publish games automatically. Navigate to General Settings to select a cron interval.', 'myarcadeplugin' ); ?></i></td>
        </tr>
 ...
 */
function myarcade_get_fetch_options_gamemonetize() {

	// Get distributor settings.
	$settings = MyArcade()->get_settings( 'gamemonetize' );
	$defaults = myarcade_default_settings_gamemonetize();
...
function myarcade_feed_gamemonetize( $args = array() ) {
  ...
  if ( ! empty( $settings['limit'] ) ) {
	...
	// Generate Feed URL.
	$default_settings = myarcade_default_settings_gamemonetize();
      ...
      foreach( $categories as $gamecat ) {
        ...
        foreach ( $feedcategories as $feedcat ) {
            ...
            if ( ! empty( $gamemonetize_categories[ $feedcat['Name'] ] ) ) {
							...
							if ( true === $gamemonetize_categories[ $feedcat['Name'] ] ) {
